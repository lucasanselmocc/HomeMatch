...
class DatingAIAnalyzer(AbstractAIAnalyzer):
    """Analisador real de fotos de perfil de relacionamento, via Gemini."""
    ...
    def _get_gemini_analyzer(self) -> GeminiAIAnalyzer:
        if self._gemini_analyzer is None:
            self._gemini_analyzer = GeminiAIAnalyzer(config=DatingPhotoAnalysisConfig())
    ...
    def analyze_photo(self, photo: Any, prompt: str | None = None) -> list[dict]:
        try:
            ...
            logger.warning(
                "Falha na análise de IA (Dating) para a foto %s; "
