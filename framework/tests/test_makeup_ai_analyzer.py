"""
Testes para MakeupAIAnalyzer (framework/instances/makeup/strategies/ai_analyzer.py).
...
from framework.instances.makeup.strategies.ai_analyzer import (
    MakeupAIAnalyzer,
...
def test_delegates_to_gemini_and_returns_real_attributes_on_success():
    ...
    expected = [{"attribute_token": "finish.matte", "strength": 0.92}]
    fake = _FakeGeminiAnalyzer(result=expected)
    analyzer = MakeupAIAnalyzer(gemini_analyzer=fake)
    photo = DemoObject(id=1, image="base.jpg")

    attributes = analyzer.analyze_photo(photo, prompt="Analise esta base.")

    assert attributes == expected
    assert fake.calls == [(photo, "Analise esta base.")]
...
def test_falls_back_to_demo_attributes_when_gemini_call_fails():
    ...
    analyzer = MakeupAIAnalyzer(gemini_analyzer=fake)
    photo = DemoObject(id=2, image="base.jpg")
...
def test_default_construction_wires_up_the_real_photo_analysis_config(monkeypatch):
    ...
    GeminiAIAnalyzer real, configurado com MakeupPhotoAnalysisConfig — não
    ...
    assert isinstance(gemini_analyzer.config, MakeupPhotoAnalysisConfig)


def test_missing_api_key_falls_back_instead_of_crashing(monkeypatch):
    """
    Regressão: sem GEMINI_API_KEY no ambiente, MakeupAIAnalyzer() não deve
    lançar exceção ao ser construído (isso quebraria `demo.py` e
    `create_makeup_app()` de imediato). A falha só deve aparecer, de forma
    ...
    analyzer = MakeupAIAnalyzer()  # não deve lançar
    photo = DemoObject(id=3, image="base.jpg")
